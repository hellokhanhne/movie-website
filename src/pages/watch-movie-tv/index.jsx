import { faArrowRight, faPlus, faStar, faThumbsDown, faThumbsUp, faThumbTack } from "@fortawesome/free-solid-svg-icons";
...
import { embedMovie, embedTV, formatNumber, getImage, handleScrollToTop } from "utils";
...
const WatchMovieTv = () => {
    ...
    const handleHideComment = async (data) => {
        const newDataComment = dataComment.comment.filter((item) => item.id_comment !== data.id_comment);
    ...
    const handleHideReply = async (data) => {
        const newDataReply = dataReply.reply.filter((item) => item.id_reply !== data.id_reply);
    ...
    useLayoutEffect(() => {
        const handleGetData = async () => {
            ...
            if (currentType === "tv") {
                ...
                await handleGetEpisodeTv(idDetail, season.currentSeason, season.currentEpisode, {
                    api_key: process.env.REACT_APP_API_KEY,
                    language: locale,
                    append_to_response: "videos",
                });
    ...
    useEffect(() => {
        const getNameCurrentUser = async () => {
            ...
            const queryUser = query(collection(dbfireStore, "user"), where("user_id", "==", dataUser.uid));
    ...
    useEffect(() => {
        const handleGetdata = async () => {
            ...
            const querySnapsotComment = query(collection(dbfireStore, "detail"), where("id_detail", "==", idDetail));
            const querySnapsotReply = query(collection(dbfireStore, "reply"), where("id_detail", "==", idDetail));
            const querySnapsotReaction = query(collection(dbfireStore, "reaction"), where("id_detail", "==", idDetail));
            ...
            const checkReplyReactionExits = doc(dbfireStore, "reaction_reply", idDetail);
            ...
            onSnapshot(querySnapsotComment, (querySnapshot) => {
                querySnapshot.forEach((docs) => {
                    setDataRate((pre) => {
                        const result = handleGetRateCurrent(docs.data().listRate, dataUser.uid);
    ...
    const handleAddRateMovie = async (value) => {
        ...
        if (isLogin) {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    const isExistRate = handleGetRateCurrent(docSnap.data().listRate, uid);
                    const dataAdd = {
                        id_rate: uid,
                        value_rate: value,
                    };
            ...
            handlePopupNotification("You need to login to perform this function", "warning");
        }
    };
            ...
            >
                <Col span={24}>
                    ...
                    {isLoading ? (
                        ...
                        >
                            <Tooltip title={t(`Open similar ${handleGenerateText(pathname)}`)}>
                                <p
                    ...
                    <div className="w-full mt-10">
                        <BodyWatch
                            ...
                            listCastsMovie={
                                tabGlobal === "/"
                                    ? listCastsMovie
                                    : dataEposideTv?.guest_stars?.filter((item, index) => index < 8 && item)
                            }
                        ...
                        {tabGlobal === "/" ? (
                            ...
                            <div className="flex flex-col justify-end items-end px-[24px]">
                                <p className="italic font-bold text-[18px]">
                                    {t("Episode")}&nbsp;
                                    <span className="italic text-[18px]">{season.currentEpisode}</span>
                                </p>
                                <div className="flex">
                                    <p className="text-[18px]">
                                        {t("Season")} <span className="text-[18px]">{season.currentSeason}</span>
                                    </p>
                ...
                {tabGlobal === "/" ? (
                    <div className="h-[100%]">
                        <div className="flex justify-between items-center mb-4">
                            <p id="similar-movie" className="text-white uppercase text-[18px] xl:text-[22px]">
                                {t("Similar")}
                            </p>
                            <div
                                onClick={() => {
                            ...
                            >
                                <FontAwesomeIcon icon={faArrowRight} className="cursor-pointer text-white" />
                            </div>
                        </div>
